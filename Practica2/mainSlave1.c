...
const char cmd1[] = {"5001AA"}; 
const char cmd2[] = {"5000AA"};
const char cmd3[] = {"5101AA"}; 
const char cmd4[] = {"5100AA"}; 
...
{   
    ...
    while (1) {
        if (comando_detectado) {
            if (!strcmp(((const char*) dataCMD), cmd1)) LATAbits.LATA1 = 1;
            if (!strcmp(((const char*) dataCMD), cmd2)) LATAbits.LATA1 = 0;
            if (!strcmp(((const char*) dataCMD), cmd3)) {
                Allowprint = 1;
                flag = 1;
            }
            if (!strcmp(((const char*) dataCMD), cmd4)) {
                Allowprint = 0;
                flag = 1;
            }
        ...
        if ((Allowprint)&&(BufferLoadDone)&&(U1STAbits.TRMT)) 
        {
            if (U1_PrintRate++ >= 4) 
            {
                ...
                sprintf(txbuffer, "Imprimiendo datos: %d\n", contador++);
        ...
        {
            memset(txbuffer, '\0', sizeof (txbuffer));
            sprintf(txbuffer, "Se ha interrumpido la impresion de datos\n");
